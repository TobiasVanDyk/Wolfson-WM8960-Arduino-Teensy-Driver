...
Proto-driver (eventually as a library), and example applications for Arduino and Teensy MCUs (Note 1), using the Wolfson WM8960 Audio DAC, with separate stereo BTL 1W speaker and headphone amplifiers (common grounded - Note 2), and a fractional-enabled PLL (which is missing from the the severly restricted WM8731 DAC), which enables the use of one external crystal supporting allmost all, commonly-used audio clocking schemes (Note 2). 
